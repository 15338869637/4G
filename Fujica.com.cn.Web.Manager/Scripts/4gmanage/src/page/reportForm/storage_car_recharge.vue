<template>
  <div class="storage-car-recharge flex-box column">
    <div class="main box">
            ...
            <div class="condition">
                ...
                <div class="condition-con">
                    <el-select v-model="form.PaymentType" class="w-input" placeholder="请选择">
                        <el-option label="全部" value=""></el-option>
                        <el-option
                            v-for="item in options.PaymentType"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value">
                        </el-option>
      ...
      <div class="table flex1 auto">
        <el-table
          ...
          <el-table-column
            ...
            label="收款/退款金额">
            <template slot-scope="scope">
              <span :class="(scope.row.OperatType == '退卡' ? 'red' : 'green')">{{scope.row.RechargeAmount}}</span>
            </template>
          </el-table-column>
          <el-table-column
  ...
  methods: {
    ...
    filterData(list) {
      return list.map(obj => {
            ...
            if (item.OperatType !== 2) {
              item.oldCurrentBalance = (item.CurrentBalance * 100 + item.RechargeAmount * 100) / 100
            } else {
              item.oldCurrentBalance = 0
            }
            // 支付方式
            ...
            item.RechargeAmount = (item.OperatType == 2 ? '-' : '') + item.RechargeAmount
            item.OperatType = this.options.OperatType[item.OperatType]
            item.CurrentBalance = item.CurrentBalance.toFixed(2)
            item.oldCurrentBalance = item.oldCurrentBalance.toFixed(2)
            return item
          })
    },
    // 支付方式
    ...
    async exportExcel () {
      ...
      let title = ['LicensePlate','CarType','city','address','OperatType','zip','zip','PaymentType','RechargeAmount','RechargeOperator','CreateTime','Remarks']
      let config = {
        header: tHeader,
        autoWidth: true
      }
      ...
      export_json_to_excel({
        list,
        filename: "储值车缴费记录报表" + arr[0] + '-' + arr[arr.length - 2]
      });
      this.loading = false
    },
    Delay (item) {
      // 到期时间 续费类型 续费数量
      let {ExpirationTime, RenewalType, RenewalValue} = item
      switch (RenewalType) {
        case 1:
          return {
            start: new Date(ExpirationTime).getTime() - (RenewalValue * 24 * 60 * 60 * 1000),
            end: ExpirationTime
          }
          break;
      }
      return {}
      console.error('数据有误！', 'RenewalType:' + RenewalType)
    },
    // 车类
    async getCarType() {
