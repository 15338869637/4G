      ...
      <div class="month-card-wrap">
        ...
        <div class="table">
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button class="parkButton" size="small" @click='operatePush(0,JSON.stringify(scope.row),getCardName(scope.row.CarTypeGuid))'  type="text" >充值</el-button>
              <el-button class="parkButton" size="small" v-show='parkState(scope.row.Enable,scope.row.Locked)=="正常"' @click='operatePush(1,JSON.stringify(scope.row),getCardName(scope.row.CarTypeGuid))' type="text">锁定</el-button>
              <el-button class="parkButton" size="small" v-show='parkState(scope.row.Enable,scope.row.Locked)=="锁定"' @click='operatePush(2,JSON.stringify(scope.row),getCardName(scope.row.CarTypeGuid))' type="text">解锁</el-button>
              <el-button class="parkButton" size="small" :disabled='parkState(scope.row.Enable,scope.row.Locked)!="正常"' @click='operatePush(3,JSON.stringify(scope.row),getCardName(scope.row.CarTypeGuid))' type="text" >注销</el-button>
              <el-button class="parkButton" size="small" :disabled='parkState(scope.row.Enable,scope.row.Locked)!="正常"' @click='operatePush(4,JSON.stringify(scope.row))' type="text" >修改</el-button> 
...
export default {
  ...
  methods: {
    ...
    // 查询提交
    async onSubmit(){
     this.formInline.ProjectGuid=getCookie("ProjectGuid");
      ...
      console.log(this.formInline,1234)

      let Base64 = require('js-base64').Base64;
      const send= Base64.encode(JSON.stringify(this.formInline))


      const res = await rxios("GET", '/CardService/GetValueCardList', {Base64args: send})
      this.tableData=res

      console.log(this.Page, this.Size,888)
      this.mylist=this.tableData.slice(0, 10)
    },
    operatePush(state,info,carType){
      if(state==4){
        this.$router.push({path: '/SetCard',query:{info:info,carType:carType,status:3,parkCode:this.formInline.ParkingCode}});
        return
      }
      this.$router.push({path: '/ValueOperate',query:{state:state,info:info,carType:carType}});
    },
    PageChange({ page, size }) {

      console.log(page,size,1)
