      ...
      <div class="table flex1 auto">
        <el-table
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
                <el-button type="text" @click="Operate(0, JSON.stringify(scope.row), scope.row._CarTypeGuid)">充值</el-button>
                <el-button type="text" @click="Operate(scope.row.Locked ? 2 : 1, JSON.stringify(scope.row), scope.row._CarTypeGuid)">{{scope.row.Locked ? '解锁' : '锁定'}}</el-button>
                <el-button type="text" @click="Operate(3, JSON.stringify(scope.row), scope.row._CarTypeGuid)">注销</el-button>
                <el-button type="text" @click="Operate(4, JSON.stringify(scope.row), scope.row._CarTypeGuid)">修改</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    init (Request) {
      ...
      GetValueCardList(data).then(res => {
    ...
    filterData(list) {
      return list.map(item => {
            ...
            item._Balance = item.Balance.toFixed(2)
            return item
          })
    },
    Reset() {
    ...
    Operate(state, info, carType) {
      if(state==4){
        this.$router.push({path: '/SetCard',query:{info:info,carType:carType,status:3,parkCode:this.form.ParkingCode}});
        return
      }
    ...
    async getCarType() {
      ...
      this.options.CarTypeGuid = res.filter(item => item.CarType === 2).map((item, index) => {
