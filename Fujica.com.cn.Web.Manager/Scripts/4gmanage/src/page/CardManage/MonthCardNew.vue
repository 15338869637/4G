      ...
      <div class="table flex1 auto">
        <el-table
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
                <el-button type="text" @click="Operate(0, JSON.stringify(scope.row), scope.row._CarTypeGuid)">延期</el-button>
                <el-button type="text" @click="Operate(scope.row.Locked ? 4 : 3, JSON.stringify(scope.row), scope.row._CarTypeGuid)">{{scope.row.Locked ? '解锁' : '锁定'}}</el-button>
                <el-button type="text" @click="Operate(5, JSON.stringify(scope.row), scope.row._CarTypeGuid)">注销</el-button>
                <el-button type="text" @click="Operate(6, JSON.stringify(scope.row), scope.row.CarTypeGuid)">修改</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    init (Request) {
      ...
      GetMonthCardList(data).then(res => {
    ...
    Operate(state, info, carType) {
      if(state==6){
        this.$router.push({path: '/SetCard',query:{info:info,carType:carType,status:2,parkCode:this.form.ParkingCode}});
        return
      }
    ...
    async getCarType() {
      ...
      this.options.CarTypeGuid = res.filter(item => item.CarType === 1 || item.CarType === 3).map((item, index) => {
