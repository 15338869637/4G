  ...
  <div class="card-operate-wrap">
    ...
    <div class="card-box">
      ...
      <div class="card-info">
          ...
          <!-- 如果是注销 cardState 5 -->
          <div class="info-row" v-if='cardState==5'> 
              <div class="info-li">
                <span class="left-label input-tip">实际退款：</span> <el-input v-model="refundAmount" placeholder="请输入内容"></el-input>
              </div>
              <div class="info-li">
      ...
      <div class="delay-box" v-if='cardState==0'>
          ...
          <div class="delay-li">
            <div class="delay-row">
              <div class="delay-input">
                <span class="left-label input-tip">延期金额：</span> <div class="right-box"><el-input v-model="payMount" :disabled="payTrue" placeholder="请输入内容"></el-input></div>
              </div>
              <div class="delay-input">
      ...
      <!-- 报停 -->
      <!-- <div class="delay-box" v-if='cardState==1'>
          <div class="delay-li">
            <el-radio v-model="renewValue" label="1">报停期满自动激活</el-radio>
            <div class="delay-row delay-pause">
              <div class="delay-input">
                <span class="left-label">报停开始日期：</span>
                <div class="right-box">
                  <el-date-picker type="date" placeholder="选择日期" v-model="startDate" value-format="yyyy-MM-dd" :disabled='renewValue==2' :picker-options="pickerOptions" style="width: 100%;"></el-date-picker>
                </div>
              </div>
              <div class="delay-input">
                <span class="left-label">报停结束日期：</span> 
                <div class="right-box">
                  <el-date-picker type="date" placeholder="选择日期" v-model="endDate" value-format="yyyy-MM-dd" :disabled='renewValue==2' :picker-options="pickerOptions" style="width: 100%;"></el-date-picker>
                </div>
              </div>
            </div>
          </div>
          <div class="delay-li">
            <el-radio v-model="renewValue" label="2">根据实际情况手动激活</el-radio>
            <div class="delay-row delay-pause">
              <div class="delay-input">
                <span class="left-label">报停开始日期：</span> 
                <div class="right-box">
                  <el-date-picker type="date" placeholder="选择日期" :disabled='renewValue==1' v-model="endDateTime" :picker-options="pickerOptions" style="width: 100%;"></el-date-picker>
                </div>
              </div>
              <div class="delay-input">
              </div>
...
export default {
  ...
  methods: {
    ...
    // 头部显示
    titleGet(){
      const titleArr = ['月卡延期','月卡报停','月卡激活','月卡锁定','月卡解锁','月卡注销']
    ...
    //     if (d.getMonth() != nextDay.getMonth()) {
    //         var resultDay = d;
    //         resultDay.setMonth(resultDay.getMonth() + addMonth + 1);
    //         resultDay.setDate(1);
    ...
    async onSubmit(){
      ...
      const url = ['MonthCardRenew','MonthCarPause','MonthCarContinue','CarLocked','CarUnLocked','CancelCar']
      const successTip = ['延期成功','报停成功','激活成功','锁定成功','解锁成功','注销成功']
      ...
      if(this.cardState==0){
        ...
        if(this.payMount!=0&&!parseFloat(this.payMount) > 0){
          this.$message({
            showClose: true,
            message: '金额格式不正确',
            type: 'warning'
        ...
        Object.assign(send,{
          PrimaryEndDate: this.startDate,
          PayAmount:this.payMount,
        ...
        if(this.renewValue==1){
          ...
          // 按时间段延期
          Object.assign(send,{
            StartDate:this.cardInfo.EndDate, // this.startDate
        ...
        //console.log(moment(this.timeTransfer(this.cardInfo.EndDate)).add(this.month,'months').format('YYYY-MM-DD'))
          // const threeMonth=moment(this.timeTransfer(this.cardInfo.EndDate)).add(this.month,'months').format('YYYY-MM-DD')
      ...
      if(this.cardState==1){
        ...
        if(this.renewValue==2){
          // 根据实际情况手动激活
          const threeMonth=new Date(new Date().setMonth(new Date().getMonth()-1))
           Object.assign(send,{
           StartDate:this.startDate,
          })
  ...
  created () {
    ...
    if(this.cardState==0){
      ...
      this.endDate=this.getNextDate(this.cardInfo.EndDate,1)
