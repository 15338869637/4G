...
namespace HandlingEntryData
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(string.Format("入场数据处理工具运行中:"));
            //队列名与路由名一致
            m_rabbitMQ.PeekMessageByRabbitMQ("4GCameraEntryDataQueue", "4GCameraEntryDataQueue",
            (string content) => {

                Console.WriteLine(string.Format("收到入场数据:{0}", content));
                Console.WriteLine("");
                ...
                if (inmodel != null)
                {
                    ...
                    {
                        m_ilogger.LogFatal(LoggerLogicEnum.Tools, "", entrymodel.ParkingCode, "", "Fujica.com.cn.BroadcastService.JobsDetails.MonitorEnterData", "保存入场数据到redis异常", ex.ToString());
